...
PRESET_PROMPT = dedent(
    ...
    任意で【日別気温CSV】、および【前年同月の月報CSV】（ファイル名例：月報202410.csv）です。
    ...
    **1. T1（最大電力）とT2（電力量）の完全分離**
       - 画像内でこれらが隣接していたり、同じグラフエリアに描画されていたとしても、**必ず別々の見出し（###）を立てて、個別にコメントを生成**してください。
       - 1つの項目内で「最大電力は〇〇、電力量は〇〇」のようにまとめて記述することは**禁止**します。

    **2. T3（回路別内訳）の2段階記述（CSV連携）**
       - **現状（画像）**: 画像内のデータに基づき、シェアや順位、特徴について記述。
       - **前年比較（CSV）**: 入力に「月報yyyyMM.csv」がある場合、以下の手順で記述。
         1. CSV内の各行（回路名）について、日付列数値を合計し、その行の**月間合計値（kWh）**を算出。
         2. 画像内（T3）の上位回路名とCSV内の回路名を照合し、一致する回路について前年同月比を算出。
         3. 必ず「前年比 **+〇〇%**」や「前年同月 **〇〇 kWh** から減少」といった形で比較結果を記述。
    ...
    ### 電力量[kWh]
      今月は **18,513 kWh** となり、先月 **14,104 kWh** 比 **+31.3%**、前年7月 **19,034 kWh** 比 **-2.7%** で着地しました。冷房需要は高いものの前年より抑制傾向にあると考えられるため、上位回路の運用点検と昼休みの不要負荷停止を徹底してください。

    ### 回路別内訳（CSV連携あり）
      - **現状**: 上位5回路で **85%** を占め、最大は「3F事務所・電灯」の **2,606 kWh（22.2%）** でした。常時負荷が高くピーク・ベース双方に寄与している可能性があるため、スケジュール・在室連動の適用をご検討ください。
      - **前年比較**: 前年同月（CSV算出）と比較すると、「3F事務所・電灯」は **2,800 kWh** から **-6.9%** 減少しました。照明のLED化または消灯励行の効果が出ている可能性があるため、継続的な周知をお願いします。
...
REPORT_MONTH_PATTERN = re.compile(r"月報.*?(\d{4})(\d{2})")
...
def build_comparison_context(
    ...
    *,
    ...
    for path in paths:
        ...
        month_hint = _describe_report_month(path.name)
        header = path.name if not month_hint else f"{path.name} ({month_hint})"
    ...
    header = (
        "以下は前年比較用にアップロードされたCSVの先頭行プレビューです。"
        " 単位や区分が異なる場合は整合性に注意してください。"
    )
    return f"{header}\n\n" + "\n\n".join(sections)


def _describe_report_month(filename: str) -> str | None:
    match = REPORT_MONTH_PATTERN.search(filename)
    if not match:
        return None
    ...
    if not 1 <= month <= 12:
        return None
    prev_year = year - 1
    return f"対象月: {year:04d}-{month:02d}, 前年: {prev_year:04d}-{month:02d}"
